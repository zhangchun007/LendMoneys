...
class LogUtils {
    ...
    private static boolean sEncryptionSwitch = false; //log 是否加密
    ...
    public static void e(final Object... contents) {
        log(JSON | E, sGlobalTag, contents);
    }
    ...
    private static TagHead processTagAndHead(String tag) {
        if (!sTagIsSpace && !sLogHeadSwitch) {
            ...
            if (sLogHeadSwitch) {
                ...
                final String head = new Formatter().format("%s, %s(%s:%d)", tName,
    ...
    private static String processBody(final int type, final Object... contents) {
        ...
        if (contents != null) {
            if (contents.length == 1) {
                ...
                if (sEncryptionSwitch) {
                    body = EncryptUtil.simpleEncrypt(body);
                }
            } else {
                StringBuilder sb = new StringBuilder();
                for (int i = 0, len = contents.length; i < len; ++i) {
                    Object content = contents[i];
                    if (sEncryptionSwitch && i != 0) {
                        content = EncryptUtil.simpleEncrypt(content.toString());
                    }
                    sb.append(ARGS)
                            .append("[")
                            .append(i)
    ...
    private static void printMsg(final int type, final String tag, String msg) {
        int len = msg.length();
...
//                .append(sLogSwitch ? msg : EncryptUtil.simpleEncrypt(msg))//加密// 解密 EncryptUtil.simpleDecrypt()
                .append(msg)//加密// 解密 EncryptUtil.simpleDecrypt()
                .append(LINE_SEP);
    ...
    private static void printDeviceInfo(final String filePath) {
        String versionName = SystemUtils.getAppVersion(mApplication);
        int versionCode = SystemUtils.getAppVersionCode(mApplication);
    ...
    public static class Config {
        ...
        public Config setEncryption(final boolean encryptionSwitch) {
            sEncryptionSwitch = encryptionSwitch;
            return this;
        }
