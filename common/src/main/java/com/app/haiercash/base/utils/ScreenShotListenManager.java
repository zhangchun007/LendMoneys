 ...
 *
 * }</pre>
 * 来源 https://github.com/huiger/ScreenShotDemo <p>
 */
...
//            if (sScreenRealSize != null) {
//                Logger.d(TAG, "Screen Real Size: " + sScreenRealSize.x + " * " + sScreenRealSize.y);
//            } else {
//                Logger.w(TAG, "Get screen real size failed.");
//            }
        }
    }
     ...
     * 外部授权之后获取图片信息并且回调
     */
    public String getShotImageAndShow(Uri contentUri) {
        return handleMediaContentChange(contentUri);
    }

    /**
     * 处理媒体数据库的内容改变
     */
    private String handleMediaContentChange(Uri contentUri) {
        Cursor cursor = null;
        try {
            ...
            cursor = mContext.getContentResolver().query(
                    ...
                    null,
                    null,
                    MediaStore.Images.ImageColumns.DATE_ADDED + " desc limit 50"
            );

            if (cursor == null) {
                Logger.e(TAG, "Deviant logic.");
                return null;
            }
            if (!cursor.moveToFirst()) {
                Logger.d(TAG, "Cursor no data.");
                return null;
            }
            // 获取各列的索引
            ...
            // 处理获取到的第一行数据
            return handleMediaRowData(data, dateTaken, width, height);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     * 处理获取到的一行数据
     */
    private String handleMediaRowData(String data, long dateTaken, int width, int height) {
        if (checkScreenShot(data, dateTaken, width, height)) {
            Logger.d(TAG, "ScreenShot: path = " + data + "; size = " + width + " * " + height
                    + "; date = " + dateTaken);
            ...
            Logger.w(TAG, "Media content changed, but not screenshot: path = " + data
    ...
    private boolean checkScreenShot(String data, long dateTaken, int width, int height) {
        ...
        if (dateTaken < mStartListenTime || (System.currentTimeMillis() - dateTaken) > 30 * 1000) {
            return false;
        }
        /*
         * 判断依据二: 尺寸判断
    ...
    /**
     * 判断文件是否包含指定的关键字，如果包含说明是截屏来的文字
     *
     * @param dataPath 文件路径
     * @return true 包含截屏关键字
     */
    private boolean isKeyWordsContains(String dataPath) {
        String path = dataPath.toLowerCase();
    ...
    private boolean checkCallback(String imagePath) {
        if (sHasCallbackPaths.contains(imagePath)) {
            Logger.d(TAG, "ScreenShot: imgPath has done ; imagePath = " + imagePath);
            return true;
        }
        // 大概缓存15~20条记录便可
    ...
    public interface OnScreenShotListener {
        void requestPermissionThenShot(Uri contentUri);
    ...
    private class MediaContentObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            ...
            if (PermissionUtils.getRequestPermission(mContext, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                handleMediaContentChange(mContentUri);
            } else if (mListener != null) {
                mListener.requestPermissionThenShot(mContentUri);
            }
        }
    }
}
