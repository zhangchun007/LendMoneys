package com.haier.pulltorefreshlibrary.smart;
...
import com.haier.pulltorefreshlibrary.R;
...
public class SmartPullableLayout extends ViewGroup implements NestedScrollingParent {
    ...
    private boolean isNeedPullDownImageShow = true;
    ...
    private void preparePullablePortion() {
        if (pullDownEnabled) {
            ...
            if (mBackground != null) {
                mPullableHeader.setBackground(mBackground);
            }
        ...
        if (pullUpEnabled) {
            ...
            if (mBackground != null) {
                mPullableFooter.setBackground(mBackground);
            }
    ...
    private void updateState(int state) {
        switch (state) {
            ...
            case State.PULL_DOWN_RELEASE: {
                ...
                ivArrowPullDown.setVisibility(isNeedPullDownImageShow ? View.INVISIBLE : View.GONE);

                ivProgressPullDown.setVisibility(isNeedPullDownImageShow ? View.VISIBLE : View.GONE);
    ...
    public void reset() {
        if (currentState != State.NORMAL) {
            if (currentState <= State.PULL_DOWN_RELEASE) {
                ...
                ivProgressPullDown.setVisibility(isNeedPullDownImageShow ? View.INVISIBLE : View.GONE);
                ivArrowPullDown.setVisibility(isNeedPullDownImageShow ? View.VISIBLE : GONE);
    ...
    /**
     * @param needPullDownImageShow true :下拉时候将显示下拉的图标和加载的loadding圆圈<p/>
     *                              false : 不显示
     */
    public void setNeedPullDownImageShow(boolean needPullDownImageShow) {
        isNeedPullDownImageShow = needPullDownImageShow;
        if (ivArrowPullDown != null) {
            ivArrowPullDown.setVisibility(isNeedPullDownImageShow ? View.INVISIBLE : View.GONE);
        }
        if (ivProgressPullDown != null) {
            ivProgressPullDown.setVisibility(isNeedPullDownImageShow ? View.VISIBLE : View.GONE);
        }
    }
}
