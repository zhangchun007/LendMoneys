 ...
 * @Version: 1.0
 */
public class AlwaysFuncsAdapter extends BaseAdapter<FuncBeans, ViewHolder> {

    public AlwaysFuncsAdapter(@Nullable Object data) {
        super(R.layout.item_always_funs, data instanceof List ? (List<FuncBeans>) data : null);
    }
    ...
    protected void convert(ViewHolder viewHolder, FuncBeans funcBeans) {
        ...
        if (funcBeans != null) {
            if (!TextUtils.isEmpty(funcBeans.getItemAction())) {
                viewHolder.setVisible(R.id.layout_default, false);
                ...
                viewHolder.setVisible(R.id.layout_default, true);
