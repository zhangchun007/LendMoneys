...
public class SplitEditTextView extends AppCompatEditText {
    ...
    private boolean mShowCursor = true;//是否显示光标
    ...
    private void init() {
        ...
        mPaintCursor.setStrokeCap(Paint.Cap.ROUND);//画笔圆角效果
    ...
    public void clearFocus() {
        super.clearFocus();
        setShowCursorAndRedraw(false);
    }

    //用于失焦后去掉光标
    public void setShowCursorAndRedraw(boolean mShowCursor) {
        this.mShowCursor = mShowCursor;
        invalidate();
    }
     ...
     * 重新绘制下划线颜色
     *
     * @param colorUnderLine
     * @param colorFocus
     */
    public void setUnderlineErrorColor(int colorUnderLine, int colorFocus) {
        mUnderlineNormalColor = colorUnderLine;
        mUnderlineFocusColor = colorFocus;
        invalidate();
    }
    ...
    private void drawConnectStyle(Canvas canvas) {
        ...
        //需要减去边框的一半
        mRectFConnect.set(mBorderSize / 2, mBorderSize / 2, getWidth() - mBorderSize / 2, getHeight() - mBorderSize / 2);
    ...
    public void setInputBoxStyle(int inputBoxStyle) {
        if (inputBoxStyle != INPUT_BOX_STYLE_CONNECT && inputBoxStyle != INPUT_BOX_STYLE_SINGLE && inputBoxStyle != INPUT_BOX_STYLE_UNDERLINE) {
